...
import {clearBusketToys} from '../../actions'
import {EmptyBusket} from '../Empty-Basket'

export const Basket = () => {
    const dispatch = useDispatch();
    const {items, totalPrice, itemsCount}  = useSelector(({busket})=> busket)
    const addedToys = Object.keys(items).map(key => {
        return items[key].items[0]
    })
    const itemsToys = useSelector(({busket})=>busket.items)
    console.log('itemsToys',itemsToys)
    const clearBusket = () => {
        if(window.confirm("Вы хотите очистить корзину?")){
            dispatch(clearBusketToys())
        }
       ...
       <div>
        ...
        {itemsCount?
         (<div className="cart">
            ...
            {/* LINK */}
               <span onClick={clearBusket}>Очистить корзину</span>
              </div>
            </div>
            <div className="content__items">
              { 
                  addedToys?.map((item, index)=>(
                    <BasketItem  key={`${index}`} 
                        name={item.name} 
                        id={item.id}
                        image={item.imageUrl}
                        size={item.size}
                        price={items[item.id].allPrice}
                        countEveryToys={items[item.id].items.length}/>
                  )) 
              }
            ...
            <div className="cart__bottom">
              <div className="cart__bottom-details">
                <span> Всего игрушек: <b>{itemsCount} шт.</b> </span>
